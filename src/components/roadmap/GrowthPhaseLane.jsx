import C from "./theme";
import GrowthRoadmapNode from "./GrowthRoadmapNode";

export default function GrowthPhaseLane({ phase, index, isLast, variant = "desktop", accent = C.teal }) {
  if (variant === "mobile") {
    return (
      <div style={{ display: "flex", gap: "0.75rem", paddingBottom: "1.25rem", position: "relative" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {phase.nodes.map(node => (
            <GrowthRoadmapNode key={node.id} node={node} accent={accent} />
          ))}
        </div>
      </div>
    );
